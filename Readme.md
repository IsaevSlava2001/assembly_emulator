...
**Описание**: Вычисление суммы всех элементов массива из 6-15 элементов. Массив хранится в памяти начиная с адреса 0x1000, где первый элемент - размер массива.

**Пример кода**:
```assembly
; Сумма элементов массива
PUSH [0x1000]    ; Загружаем размер массива в стек
PUSH 0x1001      ; Загружаем базовый адрес массива в стек
PUSH 0           ; Инициализируем сумму
...
LOOP:
  ...
  DUP            ; Дублируем адрес
  LOAD           ; Читаем значение из памяти в стек
  SWAP           ; Меняем адрес и значение местами
  INC            ; Инкрементируем адрес для следующего элемента
  SWAP           ; Возвращаем адрес на место
  
  ; Суммируем
  SWAP           ; Меняем сумму и значение местами
  ADD            ; Добавляем к сумме
  SWAP           ; Возвращаем сумму на место
  
  JMP LOOP       ; Повторяем цикл

EXIT:
  POP            ; Убираем адрес со стека
